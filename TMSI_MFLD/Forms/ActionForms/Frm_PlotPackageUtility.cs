...
{
    public partial class Frm_PlotPackageUtility : System.Windows.Forms.Form
    {
        #region 字段定义
        ...
        private async void btn_Pages_Click(object sender, EventArgs e)
        {
            ...
            ////线程加载
            //Thread thread = new Thread(ZTAFileToChart);
            //thread.Start(_pathtestnumber);
        }
        ...
        private void btn_excel_Click(object sender, EventArgs e)
        {
            ...
            if (test != null && this.com_selectsteps.Text != string.Empty)
            {
                ...
                ExcelOperations.Excel(dtHead, keyValues, slope, uc_units.Text, filetype);
            ...
            if (lsx.Count > 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (!String.IsNullOrEmpty(fileName))
                    {
                        try
                        {
                            string[] filename = sfd.FileName.Split('\\');
                            SaveCSV(sfd.FileName.ToString()+".csv", filename[filename.Length-1]+".csv", lsx.ToArray(),lsy.ToArray(), dtHead);
        ...
        public void SaveCSV(string path, string fileName, string msg)
        {
            ...
            using (TextWriter fw = new StreamWriter(path, true)) // 以有序字符写入
            {
        ...
        public void ZTAFileToChart(object path)
        {
            lock (obj)
            {
                ...
                if (path.ToString() != string.Empty)
                {
                    ...
                    var dthead = JsonToDataTableHead(ztaJsonStr);

                    //设备图表类型
                    test.ChartType = SeriesChartType.Spline;
                    filetype = dthead.Rows[0]["TestType"].ToString();
                    ...
                    {
                        //获取想获取曲线的xy名称
                        x = this.dgv_PlotPackageUtility.Rows[0].Cells["Data Description"].Value.ToString();
                        y = this.dgv_PlotPackageUtility.Rows[1].Cells["Data Description"].Value.ToString();
                        double SampleFrequency = double.Parse(dthead.Rows[0]["SampleFrequency"].ToString());
                        xname = x; yname = y;
                        ...
                        if (chtTestPlot.InvokeRequired)
                        {
                            Action action = () =>
                            {
                                ...
                                try
                                {
                                    ...
                                    for (int i = 1024; i < yvaluearr.Length - 2; i++)
                                    {
                                        if (ratio != 1.0)
                                        {
                                            cache += ratio;
                                            ...
                                            if (str.Length >= 2)
                                            {
                                                ...
                                                else interval1 = int.Parse(str[0]);
                                            }
                                            else interval1 = int.Parse(str[0]);
                                        }
                                        else interval1 = i;
                                        ...
                                        //英制数据
                                        xvalue = Convert.ToDouble(Convert.ToDouble(xvaluearr[interval1]).ToString("f4"));
                                        yvalue = Convert.ToDouble(Convert.ToDouble(yvaluearr[interval1]).ToString("f4"));

                                        //判断进制
                                        switch (uc_units.Text)
                                        {
                                            case "English":
                                                break;
                                            case "SI":
                                        ...
                                        //添加曲线
                                        if (interval1 < yvaluearr.Length - 2)
                                        {
                                            if (i != 0 && i != 76023 && Convert.ToDouble(xvaluearr[interval1]) != Convert.ToDouble(xvaluearr[interval1 + 1]))
                                            {
                                    ...
                                    //写入起始值
                                    if (valueminx.XValue < valueminx.XValue.ReXMin(x, this.uc_units.Text, y))
                                    {
                                        chtTestPlot.ChartAreas[0].AxisX.Minimum = valueminx.XValue.ReXMin(x, this.uc_units.Text, y) - x.Interval(this.uc_units.Text, y);
                                        minimum = valueminx.XValue.ReXMin(x, this.uc_units.Text, y) - x.Interval(this.uc_units.Text, y);
                                    }
                                    else
                                    {
                                        chtTestPlot.ChartAreas[0].AxisX.Minimum = valueminx.XValue.ReXMin(x, this.uc_units.Text, y);
                                        minimum = valueminx.XValue.ReXMin(x, this.uc_units.Text, y);
                                    }
                                    //写入偏移
                                    chtTestPlot.ChartAreas[0].AxisX.Interval = x.Interval(this.uc_units.Text, y);
                                    interval = x.Interval(this.uc_units.Text, y);
                                }
                                catch (Exception ex)
                                {
                                    ex.ToString();
        ...
        public Dictionary<double, double[]> PercentXYValue(double[] dax, double[] day, double ymax, int mode)
        {
            ...
            for (int ii = 0; ii < 10; ii++)
            {
                ...
                if (mode == 1 && x < 0.0)
                {
                    double[] keys = keyValues.Keys.ToArray();
                    double key = keys[keys.Length - 1];
                    vs1[0] = keyValues[key][0];
                    vs1[1] = keyValues[key][1];
                }
