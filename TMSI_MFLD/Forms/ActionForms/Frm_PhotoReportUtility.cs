...
{
    public partial class Frm_PhotoReportUtility : Form
    {
        #region 字段定义
        ...
        private void btn_excel_Click(object sender, EventArgs e)
        {
                ...
                if (list < high && list < 1)
                {
                    ...
                    if (lstype.IndexOf(type) == -1)
                    {
                        ...
                        else if (this.uc_units.Text == "MKS")
                        {
                            ...
                            for (int ii = 0; ii < xanalysedata[dataRow["File Name"].ToString()].Count; ii++)
                            {
                                xanalysedata[dataRow["File Name"].ToString()][ii] = xanalysedata[dataRow["File Name"].ToString()][ii].EnglishToMKS(x);
                                yanalysedata[dataRow["File Name"].ToString()][ii] = yanalysedata[dataRow["File Name"].ToString()][ii].EnglishToMKS(y);
                            }
                        }
                        //没有时执行原始英制数据
            ...
            if (rowhead_dt.Count != 0 && excelshreet.Count != 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    Excel.ExcelOperations.Excel(datas.ToArray(), keyValuePairs, slopes, result, sfd, excelshreet.ToArray(), this.uc_units.Text, lstype, this.uc_units.Text);
        ...
        private void brn_exslr_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                string localFilePath = sfd.FileName.ToString();
                //生成报告
                ExcelOperations.Excel(localFilePath, type, this.com_unitslr.Text, lsrawtime, lsrawdata, dthead);
                //成功弹窗提示
                Frm_Sd_Excel frm_Sd_Excel = new Frm_Sd_Excel(FrmModeExcel.Success);
                frm_Sd_Excel.ShowDialog();        
            }
            ...
            switch (this.com_unitslr.Text)
            {
                ...
                case "SI":
                    ...
                    for (int ii = 0; ii < lsrawxdata.Count; ii++)
                    {
                        ...
                        yvalue.Add((lsrawdata[ii].EnglishToSi(y)) / 9.8);
                    }
                    break;
                case "MKS":
                    ibs = 100;
        ...
        public void SaveCSV(string path, string fileName, double[] XList, double[] YList, DataTable data)
        {
            ...
            {
                case "KLat":
                    if (this.uc_units.Text == "English") { sb.Append("Ibs,in"); }
                    else if (this.uc_units.Text == "SI") { sb.Append("N,mm"); }
                    else { sb.Append("N,mm"); };
                    x = "Pxy"; y = "Fy";
                    break;
                case "KLong":
                    ...
                    break;
                case "Ktor":
                    if (this.uc_units.Text == "English") { sb.Append("Ibs.M,DEG"); }
                    else if (this.uc_units.Text == "SI") { sb.Append("N.M,DEG"); }
                    else { sb.Append("N.M,°"); };
                    x = "Az"; y = "Mz";
                    break;
                case "Kv":
